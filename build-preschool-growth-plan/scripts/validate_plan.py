#!/usr/bin/env python3
"""Validate a public 3-6 annual growth plan."""
from __future__ import annotations
import argparse, re, sys
from pathlib import Path

HEADINGS=("# 3—6 岁全年成长规划","## 使用说明与边界","## 孩子、家庭与学前环境","## 证据地图","## 优势、兴趣与关系资源","## 游戏、语言与探究画像","## 动作、生活自主与日常节律","## 情绪、同伴与集体参与","## 当前支持重点","## 90 天家庭与学前行动实验","## 全年路线图","## 入学过渡与环境协作","## 复盘安排","## 需要进一步了解","## 专业支持提示")
QUARTERS=("### 第一季度","### 第二季度","### 第三季度","### 第四季度")
FIELDS=("目标","依据","孩子/成人/环境行动","频率","成功信号","停止或调整条件","记录方式")
SAFETY="本规划不替代医疗、心理、发育筛查或教育专业评估"
RISKS=(r"(?:诊断|确诊)为",r"(?<!不)(?:一定|必然)(?:会|能|可以|成为|录取)",r"(?:VARK|多元智能|左右脑).{0,12}(?:类型|定型|分数)",r"入学准备度.{0,8}(?:分|评分)",r"/" r"Users/[^\s)]+",r"(?i)\b(?:app_secret|access_token|api_key)\b\s*[:=]")
...
def sample():
    ...
    for h in HEADINGS:
        ...
        if h=="## 当前支持重点": parts += ["### 重点：集体参与","E01"]
        if h=="## 90 天家庭与学前行动实验": parts += ["### 实验：轮流游戏","**目标：** 参与更顺畅","**依据：** E01","**孩子/成人/环境行动：** 共玩并等待","**频率：** 每周三次","**成功信号：** 能继续互动","**停止或调整条件：** 明显不适","**记录方式：** 简记"]
        if h=="## 全年路线图":
