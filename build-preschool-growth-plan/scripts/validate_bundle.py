#!/usr/bin/env python3
"""Validate the preschool intake handoff."""
...
TRACK, LOW, HIGH = "preschool", 36, 72
...
def validate(data):
    ...
    try:
        ...
        if not LOW <= age < HIGH: errors.append(f"月龄 {age} 不属于 36—71 月龄")
...
def self_test():
    good={"schema_version":"junyi-child-growth-intake/v1","plan_track":"preschool","plan_as_of":"2026-07-17","child":{"birth_date":"2022-07-18","age_months":47},"family_goals":["目标"],"strengths":["优势"],"observations":[{}]*6,"attempts":["尝试"]}
    if validate(good): return 1
    good["plan_track"]="school-age"
