...
## 2026-08-31 微信公众号长文 Skill 公开转换记录

`wechat-article-writer` 来自作者长期使用的同名私人流程与真实文章改稿实践。公开版保留可迁移的 0—11 阶段协作方法、内容锁定、七类内容判断、同类型授权全文范本、大纲确认、多轮改稿、标题兑现、事实隐私、CTA 和发布证据门禁；只服务微信公众号长文。

作者本人的定位、写作 DNA、完整范文、选题库、素材库、发布规则、私人文件路径、真实人物与客户案例、业务行动邀请和内部复盘均未进入公开版，也不属于本次公开授权材料。公开版只定义外置配置契约，由每位使用者在自己的私人工作区提供资料与授权。

`junyi-article-writing-companion` 是 `v1.7.0` 中短暂发布的旧候选名称，已撤回且不作为兼容别名保留。其历史提交不会被改写；已经下载该旧目录的使用者需要自行卸载并安装唯一正式名称 `wechat-article-writer`。公开发布不自动代表作者本人生产环境已经切换。

## 2026-09-04 闪光瞬间 Skill 2.0 公开转换记录

`junyi-growth-spark-recorder` 2.0 公开版从作者长期实际使用的 `growth-spark-recorder` 私人流程和既有公开 1.0 版合并提炼。公开版只保留三层复盘、证据边界、配置契约、写入门禁和通用模型库；家庭成员姓名、生日、真实事件、内部 Agent、私人路径、自动任务账号与消息目标均不属于公开材料。

私人版后续追加的 49 个模型暂不进入公开版：它们已随原生产版完整冻结，待逐条完成去身份化、重复项和来源权利审计后再决定是否纳入。2.0.0 已通过技术测试与作者真实片段两轮盲测；公开发布不自动代表作者生产环境已经切换。
