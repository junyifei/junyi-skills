...
最后核验：2026-09-01（`wechat-article-writer` 1.0.0 候选；结构、安全和三端隔离发现已通过，真实写作客观门禁未全部通过，因此未发布、未切换生产入口）。
...
| `wechat-article-writer` 1.0.0 | 发布阻断 | 结构、安全、三端隔离发现、无配置、模拟创作者和第一轮零新增编辑门禁通过；第二轮“素材到大纲”仍出现无证据普遍化，未进入第三轮和主观盲评。详见[候选验收记录](WECHAT-ARTICLE-WRITER-VALIDATION.md) |
...
| Claude Code | 隔离发现已验证 | 项目级复制安装和独立新会话发现通过；真实写作候选仍被内容门禁阻断 |
| OpenClaw | 隔离发现已验证 | 工作区级精确加载只发现一个同名 Skill，资源完整；真实写作候选仍被内容门禁阻断 |
