...
最后核验：2026-09-04（`junyi-growth-spark-recorder` 2.0.0 已随仓库 v1.9.0 发布；结构、安全、三端隔离发现、隔离回退与真实片段两轮盲测通过。作者生产入口尚未切换，冻结旧版继续保留）。
...
| `junyi-growth-spark-recorder` 2.0.0 | 已随 v1.9.0 发布；客观与真实片段主观门禁通过 | 分离分析、事件保存、长期档案更新与对外分享权限；配置、隐私、未知日期、多人单一事件源、三端隔离发现和回退通过。作者生产入口仍使用冻结旧版，后续切换另行授权 |
