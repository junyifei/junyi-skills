...
最后核验：2026-09-06（`junyi-growth-spark-recorder` 2.0.0 已随仓库 v1.9.0 发布；结构、安全、三端隔离发现、隔离回退与真实片段两轮盲测通过。2026-09-05 已完成作者三端生产切换与回退复跑；冻结旧版继续作为不可发现的回退快照保留）。
...
| `junyi-growth-spark-recorder` 2.0.0 | 已随 v1.9.0 发布；客观与真实片段主观门禁通过 | 分离分析、事件保存、长期档案更新与对外分享权限；配置、隐私、未知日期、多人单一事件源、三端隔离发现和回退通过。2026-09-05 已完成作者三端生产切换与回退复跑，冻结旧版仅作为不可发现的回退快照保留 |
| `wechat-article-writer` 1.0.0 | 已随 v1.8.0 发布；客观与三轮主观门禁通过 | 结构、安全、三端隔离发现、无配置、模拟创作者、三轮真实写作客观门禁和回退复跑均通过；第一、第二、第三轮最终均由公开候选版胜出。作者生产入口的新会话与真实回退／恢复复跑已完成。详见[验收记录](WECHAT-ARTICLE-WRITER-VALIDATION.md) |
...
| Claude Code | 隔离与作者生产发现已验证 | 项目级复制安装、独立新会话发现和作者生产入口切换通过；冻结旧版不可发现 |
| OpenClaw | 隔离与作者生产发现已验证 | 工作区级精确加载只发现一个同名 Skill，资源完整；作者生产管理副本与公开主源哈希一致，冻结旧版不可发现 |
