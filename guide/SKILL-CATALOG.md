...
这条路径用来搭建和持续更新一个长期服务于自己家庭的家庭成长 Agent。它是超级个人 AI 工作台在孩子成长与家庭教育领域的一个子系统，按“全年知识底座 → 季度行动 → 日常真实记录 → 复盘与更正”工作。全年底座优先给 Agent 长期读取，季度计划优先给父母和孩子进入真实生活。

| 类型 | 核心任务 | 使用时机 |
|---|---|---|
| 全年知识底座 | 帮助 Agent 建立对孩子、家庭目标、资源、关系和长期方向的首次系统理解 | 第一次搭建，或整体情况发生重大变化时 |
| 季度行动指南 | 把全年方向和最近新证据转成未来 90 天的聚焦、最低版本和复盘信号 | 已有全年底座，准备进入下一季度时 |
...
### 第一次建立家庭成长 Agent 的全年知识底座

先采集资料，再按计划日期完整月龄进入一个报告 Skill；三个年龄报告 Skill 不混用。

| 年龄与任务 | 使用入口 | 准备什么 | 会得到什么 |
|---|---|---|---|
| 0—12 岁首次采集、导入、补充或审核资料 | [`junyi-child-annual-intake`](../junyi-child-annual-intake/SKILL.md) | 计划日期、孩子基本情况与年龄信息、家庭观察；按需要补孩子或教师视角 | `intake.json`、证据地图、完整度和唯一年龄路由 |
...
季度行动指南需要兼容的全年 `intake.json`、完整全年规划和状态为 `ready` 的本季更新。没有持续记录时可以填写适龄季度问卷。
