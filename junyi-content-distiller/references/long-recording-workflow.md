...
在使用者指定的工作目录下建立独立任务目录；未指定时使用当前目录下的 `.junyi-content-distiller/<日期或任务名>/`：
...
├── names.md             # 可选：使用者确认的人名变体与待确认项
├── unconfirmed-names.md # 可选：表外疑似人名及原文证据
├── manifest.json        # 断点状态
└── final.md             # 验收通过的最终稿
```

不得覆盖使用者原文件。原始副本与中间产物必须实际存在，不能只保留在对话上下文。
...
- 变体撞名、数字连读或无法消歧时保持未知。
- 表外疑似人名写入 `unconfirmed-names.md`，记录疑似词、原文上下文和证据位置；等待使用者确认，不自动修改知识库或人物档案。
