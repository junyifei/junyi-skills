#!/usr/bin/env python3
"""Validate a public 6-12 annual growth plan."""
from __future__ import annotations
import argparse, re, sys
from pathlib import Path

HEADINGS=("# 6—12 岁全年成长规划","## 使用说明与边界","## 孩子、家庭与学校情境","## 孩子的声音","## 证据地图","## 优势、兴趣与关系资源","## 学习与探究过程画像","## 自我管理、生活节律与数字环境","## 情绪、同伴与家庭关系","## 当前支持重点","## 90 天共同行动实验","## 全年路线图","## 家庭与学校协作","## 复盘安排","## 需要进一步了解","## 专业支持提示")
QUARTERS=("### 第一季度","### 第二季度","### 第三季度","### 第四季度")
FIELDS=("目标","依据","孩子行动","成人与环境行动","频率","成功信号","停止或调整条件","记录方式")
SAFETY="本规划不替代医疗、心理、发育筛查或教育专业评估"
RISKS=(r"(?:诊断|确诊)为",r"(?<!不)(?:一定|必然)(?:会|能|可以|成为|考入|录取)",r"(?:VARK|多元智能|左右脑).{0,12}(?:类型|定型|分数)",r"智力.{0,8}(?:上限|定型|证明)",r"/" r"Users/[^\s)]+",r"(?i)\b(?:app_secret|access_token|api_key)\b\s*[:=]")
...
def validate(text):
    ...
    if re.search(TYPICALITY,text):
        warnings.append("检测到正向发育或心理定性（如“不是异常/属于正常”）：报告不得从正反任一方向替代专业判断。")
...
def sample():
    ...
    for h in HEADINGS:
        parts += [h,"正文"]
        if h=="## 孩子的声音": parts += ["孩子希望保留自由阅读时间。"]
        if h=="## 证据地图": parts += ["观察事实｜来源｜情境｜解释或假设｜置信度","E01"]
        if h=="## 当前支持重点": parts += ["### 重点：任务开始","E01"]
        if h=="## 90 天共同行动实验": parts += ["### 实验：任务启动","**目标：** 更顺畅开始","**依据：** E01","**孩子行动：** 选择第一步","**成人与环境行动：** 减少提示","**频率：** 每个上学日","**成功信号：** 能自主开始","**停止或调整条件：** 冲突增加","**记录方式：** 简记"]
        if h=="## 全年路线图":
