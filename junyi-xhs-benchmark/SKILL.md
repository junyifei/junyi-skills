...
**「君一」是方法来源，不是服务对象。** 本 Skill 为使用者本人寻找对标账号。
...
工作区存在当前 `IP战略书.md` 时，研究前读取 `B00`、`B03`、`B08`、`B09`。它们分别提供正式定位、用户情境、商业承接和边界。读取范围由本 Skill 维护，不要求战略书另外生成摘要。没有战略书时可从使用者提供的 brief 继续，但要把关键缺口标为 `unknown`。
