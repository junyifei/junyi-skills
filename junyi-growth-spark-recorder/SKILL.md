...
- 像一位了解家庭、判断克制的朋友在复盘，不写成审计报告或冷冰冰的行为清单。
- 在事实记录之后，先用一句话指出“这段最值得看见的转折是什么”，再展开发展观察；事实边界放在相关判断旁边，不反复用免责声明打断阅读。
- 允许有温度和画面感，但温度来自材料中的具体动作、原话和互动变化，不来自替孩子解释内心。
- 当一个场景确有多个互补角度时，可使用 2—3 个模型分别照见身体状态、沟通方式和关系收场；每个模型必须增加新信息，不能换术语重复同一个判断。
- 给家长的建议要自然、可说出口。优先提供一句当场能说的话或一个小动作，不把闪光瞬间改造成训练任务。
...
| 孩子提出抱抱、陪伴、暂停说话等支持请求 | 孩子提出了该请求；成人明确回应了什么 | 在材料未确认动作发生时，写成成人已经拥抱、陪伴或完成了支持 |
...
- 先用自然语言点明本次互动最值得看见的变化或转折，再进入分析；不要只列“信号一、信号二”。
...
- 情绪强烈、孩子反复拒绝语言输入、随后重新连接的场景，优先依次考虑 `身体先于认知（神经科学/Porges）`、`非暴力沟通(Rosenberg)`、`关系修复(Gottman/Tronick)`；只有材料确实以选择权为关键转折时，才使用 `自主性理论(Deci & Ryan)`。
- 每个模型各给一个最贴近该角度的动作或话术。若模型建议已经构成完整下一步，结尾只做一句综合，不再重复列训练清单。
...
- 孩子提出支持请求，不等于成人已经执行。材料只出现“抱一下”等请求时，只写“提出拥抱请求”；必须有动作描述或明确回应，才能写“获得拥抱／成人提供拥抱”。
...
3. 是否把孩子提出的支持请求写成成人已执行该动作？若材料没有确认执行，退回“提出请求”，并把是否发生标为未知。
4. 是否把一次事件写成稳定品质、能力、关系或因果？若是，降为本次观察与后续验证问题。
5. 模型名是否与参考标题逐字一致？不一致就纠正；无法确认就不使用。
6. 是否出现上方高频事实转换表“禁止写”的内容？出现就删除，不得仅改成“可能”。
